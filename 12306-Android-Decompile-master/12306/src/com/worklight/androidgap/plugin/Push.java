...
public class Push extends CordovaPlugin
{
  ...
  private boolean subscribe(JSONArray paramJSONArray, final CallbackContext paramCallbackContext)
  {
    ...
    try
    {
      ...
      label27: if (this.onRegisterReceiver == null)
        this.onRegisterReceiver = new BroadcastReceiver()
        {
          public void onReceive(Context paramAnonymousContext, Intent paramAnonymousIntent)
          {
            Push.access$202(Push.this, paramAnonymousIntent.getStringExtra("registrationId"));
      ...
      if (this.onRegisterErrorReceiver == null)
        this.onRegisterErrorReceiver = new BroadcastReceiver()
        {
          public void onReceive(Context paramAnonymousContext, Intent paramAnonymousIntent)
          {
            String str1 = paramAnonymousIntent.getStringExtra("errorId");
            try
            {
        ...
        if (this.onMessageReceiver == null)
          this.onMessageReceiver = new BroadcastReceiver()
          {
            public void onReceive(Context paramAnonymousContext, Intent paramAnonymousIntent)
            {
              ...
              {
                Push.this.pending.add((WLGCMIntentService.Message)paramAnonymousIntent.getParcelableExtra("message"));
                Push.this.dispatchPending();
                if (!paramAnonymousIntent.getBooleanExtra("notificationBar", false))
                  setResultCode(-1);
                return;
              }
            }
