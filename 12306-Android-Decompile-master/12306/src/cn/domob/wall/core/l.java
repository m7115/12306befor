      ...
      try
      {
        if ((v.b(paramContext)) && (v.c(paramContext)))
        {
          ...
          Arrays.sort(arrayOfScanResult, new Comparator()
          {
            ...
            {
              int i = paramAnonymousScanResult2.level - paramAnonymousScanResult1.level;
              int j;
              if (i > 0)
                j = 1;
              do
  ...
  protected static String a(Context paramContext)
  {
    if (b == null)
    {
      ...
      if (!v.g(b))
      {
        ...
        Log.i("DrwSDK", "SDK_VERSION:010100,SDK_BUILD_DATE:20131031");
      }
    }
    return b;
  }
  ...
  protected static String i(Context paramContext)
  {
    if (h == null)
      if (Build.VERSION.RELEASE.length() <= 0)
        break label34;
    label34: for (h = Build.VERSION.RELEASE.replace(",", "_"); ; h = "1.5")
      return h;
  }
